...
def translate_to_fa(text: str) -> str:
    """ترجمه به فارسی با Gemini (با مدیریت rate limit) + فال‌بک Google"""
        ...
        for model in models:
            for attempt in range(3):
                try:
                    ...
                    prompt = (
                        "Translate this English film/TV news text into natural fluent Persian. "
                        "Keep actor names, show titles and company names as commonly written in Persian media. "
                    ...
                    time.sleep(1.2)  # جلوگیری از 429
                    if result and re.search(r"[\u0600-\u06FF]", result):
                        return result
                    break  # مدل جواب داد ولی فارسی نبود → مدل بعدی
                except HTTPError as e:
                    if e.code == 429:
                        wait = 8 * (attempt + 1)
                        print(f"[WARN] Gemini 429 on {model}, wait {wait}s...")
                        time.sleep(wait)
                        continue
                    print(f"[WARN] Gemini HTTP {e.code} on {model}: {e}")
                    break
                except Exception as e:
    ...
    if HAS_FALLBACK:
        try:
            ...
            time.sleep(0.5)
            if result and re.search(r"[\u0600-\u06FF]", result or ""):
                return result.strip()
        except Exception as e:
            print(f"[WARN] Google fallback: {e}")
...
def main():
    ...
    print(f"Gemini: {'YES' if HAS_GEMINI else 'NO (fallback only)'}")
    all_items = []
