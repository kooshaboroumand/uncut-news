...
def translate_to_fa(text: str) -> str:
    """ترجمه به فارسی با Gemini (اولویت) و فال‌بک به MyMemory/Google"""
    ...
    if HAS_GEMINI:
        try:
            ...
            prompt = (
                "Translate the following English entertainment/film news text into natural, fluent Persian (Farsi). "
                "Keep proper names (actors, shows, companies) in their common form. "
                "Do not add explanations. Only output the Persian translation.\n\n"
                + chunk
            )
            body = _json.dumps({
            ...
            if result and re.search(r"[\u0600-\u06FF]", result):
                return result
            print("[WARN] Gemini returned empty/non-Persian")
        except Exception as e:
